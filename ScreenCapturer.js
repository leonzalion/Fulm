...
class ScreenCapturer {
  ...
  stopRecording() {
    ...
    (async () => {
      ...
      this.saveWindow.window.on('closed', () => {
        if (this.screenshotBoxResponse === 0) {
      ...
      this.saveWindow.window.on('close', async (e) => {
        if (shouldSaveWindowExit) {
          ...
          const messageBox = await dialog.showMessageBox({
            ...
            buttons: ['Yes', 'Cancel'],
            title: 'Confirm',
            message: `Are you sure you want to exit? (Make sure you've saved your time-lapse!)`
          });

          this.messageBoxResponse = messageBox.response;
          if (messageBox.response !== 2) {
            const saveScreenshotBox = await dialog.showMessageBox({
              type: 'question',
              buttons: ['Yes', 'No', 'Cancel'],
              title: 'Screenshots',
              message: `Would you like to delete this time-lapse's screenshots (located at "${this.sessionScreenshotDir}")?`
            });
            this.screenshotBoxResponse = saveScreenshotBox.response;
            if (saveScreenshotBox.response !== 2) {
  ...
  saveRecording() {
    ...
    let saveProgressWindowOptions = {
      file: path.join(app.getAppPath(), 'renderer/saveProgressWindow/index.html'),
      height: 50,
      width: 500,
      showOnReady: true,
      frame: false,
    ...
    ffmpeg.on('exit', () => {
      ...
      this.saveWindow.window.webContents.send('timeLapseSaved');
      ffmpeg.kill('SIGINT');
